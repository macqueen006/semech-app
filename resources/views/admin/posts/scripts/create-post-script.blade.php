<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ════════════════════════════════════════════════════════════════════
        //
        // sessionUploadedImages  : paths uploaded in THIS browser session.
        //                          These are "owned" by this session and can
        //                          be cleaned up if the user abandons.
        //
        // currentFeaturedImagePath : the path currently set as featured image.
        //
        // currentFeaturedIsBorrowed: TRUE when the featured image was selected
        //                          from Browse Storage (i.e. it already exists
        //                          and belongs to other posts — NEVER delete it).
        //
        // originalFeaturedImagePath: the value of image_path when the page
        //                          loaded (from a savedPost draft). We must
        //                          never delete this on abandon either.
        // ════════════════════════════════════════════════════════════════════
        ...
        let currentFeaturedIsBorrowed = !!currentFeaturedImagePath; // if pre-loaded, treat as borrowed
        ...
        function autoSave() {
            ...
            if (currentSavedPostId) formData.set('saved_post_id', currentSavedPostId);
            ...
            fetch('{{ route('admin.posts.auto-save') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        if (data.saved_post_id) currentSavedPostId = data.saved_post_id;
