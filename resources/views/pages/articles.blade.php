                    ...
                    <div role="list" class="news-wrap">
                        @foreach($posts as $index => $post)
                            @php
                                ...
                                // Using pagination methods instead of manual $offset
                                $globalPosition = (($posts->currentPage() - 1) * $posts->perPage()) + $index + 1;
                            @endphp
                    ...
                    @if($posts->hasPages())
                        <div role="navigation" aria-label="List" class="pagination-wrapper">
                            @if($posts->onFirstPage() === false)
                                <a href="{{ $posts->previousPageUrl() }}" aria-label="Prev Page" class="pagination-btn">
                                    <div class="inline-block">Previous</div>
                                </a>
                            @endif

                            @if($posts->hasMorePages())
                                <a href="{{ $posts->nextPageUrl() }}" aria-label="Next Page" class="pagination-btn">
